...
bool inorderBST (TreeNode* root,int& prev) {
    if (root == 0) {
        ...
        prev = root->val;
        bool right_b = inorderBST (root->right,prev);
...
bool isValidBSTHelper(TreeNode *root,int minVal,int maxVal) {
    if (!root) {
        ...
        if (root->val > minVal && root->val < maxVal && isValidBSTHelper(root->left,minVal,root->val) && isValidBSTHelper(root->right,root->val,maxVal)) {
            return true;
        } else {
            return false;
        }
...
//Determine max height with BFS O(n)
...
//Find the min height of BST with BFS 
int find_min_height (TreeNode *root) {
    ...
    if (root) {
        ...
        while (!t_queue.empty()) {
            ...
            //Add the element to queue and increase nodes_to_visit
            if (tmp->left) {
                t_queue.push(tmp->left);
                nodes_to_visit += 1;
            }
            ...
            //Remove the top and reduce the curr_visited
            t_queue.pop();
            curr_visited--;
            ...
            //If the curr_visited is 0 bottle with zero element swap the content in the bottle
            if (curr_visited == 0) {
                min_height += 1;
                curr_visited = nodes_to_visit;
                nodes_to_visit = 0;
